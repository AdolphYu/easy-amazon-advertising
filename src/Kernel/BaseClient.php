...
//        $this->client = new Client(['handler' => $handlerStack]);
    ...
    public function httpGet(string $url, array $data = [], $isVersion = true, $headers = [])
    {
        ...
        return $this->request($requestUrl.$url, 'GET', ['query' => $data, 'headers' => $headers, 'timeout' => 600, 'proxy' => ['http'  => "http://210.16.120.235:10021"], 'debug' => true]);
    }
...
//        var_dump($headers);exit;
        $requestUrl = $isVersion ? $this->apiEndpoint : $this->apiNoVersionEndpoint;
        return $this->request($requestUrl.$url, 'POST', ['query' => $query, 'json' => $data, 'headers' => $headers, 'timeout' => 600, 'proxy' => ['http'  => "http://210.16.120.235:10021"], 'debug' => true]);
    }
